<template>
  <section class="bg-surface-container border border-outline-variant rounded-xl">
    <div class="row q-col-gutter-sm items-stretch">
      <div class="col-12 col-sm relative">
        <input
          v-model="query"
          class="full-width bg-background border border-outline-variant rounded-lg px-4 py-2 text-on-surface focus:ring-2 focus:outline-none transition-all placeholder:text-on-surface-variant"
          style="font-family: 'Geist'; font-size: 14px; line-height: 1.5"
          placeholder="Enter orchestration prompt..."
          type="text"
        />
        <div class="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-2 gt-xs">
      ...
      <div class="col-12 col-sm-auto">
        <button
          @click="executeQuery"
          class="full-width px-3 py-2 bg-primary text-on-primary font-bold rounded-lg flex items-center justify-center gap-2 hover:brightness-110 active:scale-95 transition-all shadow-lg"
